...
<head>
    ...
    <style>
        :root {
            ...
            --glass-bg-heavy: rgba(16, 18, 24, 0.9);
        ...
        * { margin:0; padding:0; box-sizing:border-box; }
        html, body, #root { height:100%; overflow:hidden; background:var(--bg-deep); color:var(--text-primary); font-family:'gg sans','Noto Sans','Helvetica Neue',Arial,sans-serif; font-size:14px; }
        ...
        .btn-icon:hover { background: var(--glass-hover); color: var(--text-primary); transform: scale(1.1); }
        ...
        .btn-danger:hover { background: rgba(242,63,66,0.4); }
        ...
        .session-item {
            ...
            margin: 2px 0;
            cursor: pointer;
            transition: background 0.25s, transform 0.25s, border-left-color 0.2s, padding-left 0.2s;
        ...
        .message-bubble {
            max-width:72%; padding:12px 16px; border-radius:20px; font-size:14px; line-height:1.55;
        ...
        .bubble-user { background: rgba(255,255,255,0.1); border-bottom-left-radius:4px; }
        .bubble-ai { background: rgba(88,101,242,0.3); border-bottom-right-radius:4px; color:#fff; }
        .bubble-system { background: rgba(255,255,255,0.04); color:var(--text-secondary); font-size:12px; text-align:center; border-radius:30px; max-width:80%; margin:0 auto; }
        @keyframes bubbleIn {
...
<body>
    ...
    <script type="text/babel">
        ...
        function App(){
            ...
            useEffect(()=>{
                const s=io(window.location.origin,{ transports:['polling'], reconnection:true, reconnectionDelay:2000 });
                socketRef.current=s;
